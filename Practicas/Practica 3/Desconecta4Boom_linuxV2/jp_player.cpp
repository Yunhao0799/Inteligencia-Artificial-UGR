...
double Player::Valoracion(const Environment &estado, int jugador){
   ...
   if (ganador==jugador)
      ...
      return MiPuntuacion(estado,contrario) - MiPuntuacion(estado,jugador) + MiPuntuacion(estado, contrario) - MiPuntuacion(estado, jugador);
   }
}
...
Environment::ActionType Player::Think(){
   ...
   //       }
    ...
    valor = Minimax(this->actual_, jugador_, 4, accion);
    cout << "Valor MiniMax: " << valor << "  Accion: " << actual_.ActionStr(accion) << endl;
   // bool acts[8];
