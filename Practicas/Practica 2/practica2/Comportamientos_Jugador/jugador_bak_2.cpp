...
Action ComportamientoJugador::think(Sensores sensores) {
  //Se inicializa la variable action que devolverá (así si es el nivel 2, se queda quieto)
  ...
  if (sensores.nivel != 4){
    //Actualizo el valor de los sensores (ya que devuelven -1 si no tienen información)
    /*Si sensores devuelve -1, quiere decir que está
    andando sin chocarse, por lo tanto tendré que ir
    actualizando los valores de fil y col (con el switch de ultimaAccion)
    e ir refrescando sigAccion segun el plan, y eliminar la acción que 
    voy a seguir, de la lista del plan.
    Adicionalmente también hay que tener en cuenta si se ha modificado el plan
    en esta ejecución.
    Antes de salir, se guarda la última acción (osea, la actual).*/
    ...
    accion = sigAccion; //Normalizo lo que va a devolver
